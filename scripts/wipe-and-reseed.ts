...
async function seedDatabase(): Promise<void> {
  ...
  const { error: membershipsError } = await supabase.from("memberships").insert([
    {
      ...
      eligible_date: "2025-12-15",
      agreement_signed_at: "2020-07-15T10:00:00Z",
    ...
    },
    {
      ...
      last_payment_date: "2025-12-01",
      next_payment_due: "2026-01-10",
      eligible_date: null,
      agreement_signed_at: "2023-12-10T10:00:00Z",
    ...
    },
    {
      ...
      next_payment_due: "2025-11-05",
      eligible_date: null,
      agreement_signed_at: "2022-01-05T10:00:00Z",
      auto_pay_enabled: false,
    },
    {
  ...
  const { error: paymentsError } = await supabase.from("payments").insert([
    ...
    // Muhammad's payments
    {
      ...
      net_amount: 499.0,
      months_credited: 0,
      invoice_number: "INV-2023-0050",
      period_label: "Enrollment Fee",
      created_at: "2023-12-10T10:00:00Z",
      paid_at: "2023-12-10T10:00:00Z",
    },
    {
      organization_id: ORG_ID,
      membership_id: MEMBERSHIP_IDS.muhammad,
  ...
  const { error: agreementsError } = await supabase.from("agreements").insert([
    // Ahmed - Signed (has completed onboarding)
    {
      ...
      signed_at: "2025-11-12T14:30:00Z",
    ...
    // preferred_language: "en" -> English template
    {
      ...
      signed_at: "2025-11-28T09:15:00Z",
    ...
    // preferred_language: "fa" -> Farsi template
    {
      ...
      signed_at: "2025-10-30T11:00:00Z",
    ...
    // Omar - Signed (has pending manual onboarding)
    {
      ...
      signed_at: "2025-12-07T16:45:00Z",
  ...
  const { error: emailLogsError } = await supabase.from("email_logs").insert([
    {
      ...
      language: "en",
      status: "delivered",
      sent_at: "2025-12-01T10:01:00Z",
      delivered_at: "2025-12-01T10:01:30Z",
      resend_id: "re_abc123",
    },
    {
      organization_id: ORG_ID,
    ...
    },
    {
      ...
      status: "delivered",
      sent_at: "2025-12-10T10:00:00Z",
      delivered_at: "2025-12-10T10:00:30Z",
      resend_id: "re_ghi789",
    },
  ]);
  ...
  const { error: onboardingError } = await supabase.from("onboarding_invites").insert([
    // Stripe - Completed (Ahmed: signed agreement, completed onboarding)
    {
      ...
      enrollment_fee_paid_at: "2025-11-15T14:30:00Z",
      dues_amount: 30.0,
      billing_frequency: "monthly",
      dues_paid_at: "2025-11-15T14:30:00Z",
      planned_amount: 30.0,
      status: "completed",
    ...
    // Stripe - Expired (Fatima: signed agreement, but checkout link expired)
    {
      ...
      expired_at: "2025-11-02T09:00:00Z",
    },
    ...
    // Manual - Pending (Muhammad: signed agreement, enrollment fee paid, waiting for dues)
    {
      ...
      enrollment_fee_amount: 500.0,
      includes_enrollment_fee: true,
      enrollment_fee_paid_at: "2025-12-05T10:00:00Z",
      dues_amount: 30.0,
      billing_frequency: "monthly",
      dues_paid_at: null,
      planned_amount: 30.0,
