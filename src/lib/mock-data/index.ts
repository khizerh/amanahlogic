  ...
  memberships.forEach((membership) => {
    ...
    if (membership.enrollmentFeeStatus !== "unpaid") {
      ...
      payments.push({
        ...
        stripePaymentMethodType: enrollIsManual ? null : 'card',
    ...
    for (let i = 0; i < numPayments; i++) {
      ...
      payments.push({
        ...
        stripePaymentMethodType: isManual ? null : 'card',
...
export function getOverduePayments(): OverduePaymentInfo[] {
  ...
  getMemberships().forEach(m => {
    ...
    results.push({
      ...
      memberName: `${m.member.firstName} ${m.member.middleName ? `${m.member.middleName} ` : ''}${m.member.lastName}`,
